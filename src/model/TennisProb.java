...
public class TennisProb {
     ...
     * @param p the probability of winning point when serving
     * @param q the probability of winning point when returning serve
     * @param fiveSetter true if the match is best of five, false if it is best
     * of three
     * @return
     */
    ...
    public static double pSet(int gamesWonA, int gamesWonB, double pA, double pB, boolean tieBreak) {
        ...
        if ((gamesWonA+gamesWonB)%2 == 0){
            return pGame(0,0,pA)*pSet(gamesWonA+1, gamesWonB, pA, pB, tieBreak) +
                    (1-pGame(0,0,pA))*pSet(gamesWonA, gamesWonB+1, pA, pB, tieBreak);
        } else{
            return pGame(0,0,pB)*pSet(gamesWonA, gamesWonB+1, pA, pB, tieBreak) + 
                    (1-pGame(0,0,pB))*pSet(gamesWonA+1, gamesWonB, pA, pB, tieBreak);
        }
    }
    
    public static double pMatch(int setsWonA, int setsWonB, double pA, double pB, boolean fiveSetter, boolean tieBreak){
        if (fiveSetter){
            if (setsWonA == 3) return 1;            
            if (setsWonB == 3) return 0;
            if (setsWonA == 2 && setsWonB == 2) return pSet(0,0,pA,pB,tieBreak);
        } else{
            if (setsWonA == 2) return 1;
            if (setsWonB == 2)return 0;
            if (setsWonA == 1 && setsWonB == 1) return pSet(0,0,pA,pB,tieBreak);
        }
        if ((setsWonA+setsWonB) % 2 == 0){
            double pSetA = pSet(0,0,pA,pB,tieBreak);
            return pSetA*pMatch(setsWonA+1, setsWonB, pA, pB, fiveSetter, tieBreak)+
                (1-pSetA)*pMatch(setsWonA, setsWonB+1, pA, pB, fiveSetter, tieBreak);    
        } else{
            double pSetB = pSet(0, 0, pB, pA, tieBreak);
            return pSetB*pMatch(setsWonA, setsWonB+1, pA, pB, fiveSetter, tieBreak)+
                    (1-pSetB)*pMatch(setsWonA+1, setsWonB, pA, pB, fiveSetter, tieBreak);
        }
    }
